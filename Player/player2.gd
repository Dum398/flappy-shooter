...
@export var input_fly :StringName = "ui_accept"
@export var input_shoot :StringName = "shoot"
@export var input_bomb :StringName = "bomb"
@export var health := 3:
	set(new_health):
		health = new_health
...
func _process(delta):
	...
	if Input.is_action_pressed(self.input_shoot):
		self._shoot_now()
	
	if Input.is_action_just_pressed(self.input_bomb):
		self.try_bomb()
...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed(self.input_fly):
